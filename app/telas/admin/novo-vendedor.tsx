...
export default function AdminNovoVendedorScreen() {
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      await api.put(`/usuarios/${usuario_id}/endereco`, {
        cep: cep.replace(/\D/g, ''),
        rua,
        numero,
        bairro,
      ...
      Alert.alert('Erro', error.response?.data?.detail?.[0]?.msg || error.response?.data?.detail || 'Falha ao promover o usuário a vendedor.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          <Text style={styles.subtitle}>
            Você está criando uma vitrine para o usuário: <Text style={{fontWeight:'bold', color: '#2E7D32'}}>{usuario_nome || 'Desconhecido'}</Text>
          </Text>
          ...
          <View style={styles.form}>
            ...
            >
              ...
              <Text style={[styles.input, { color: comunidadeSelecionada ? '#333' : '#999', paddingTop: 15 }]}>
