...
export default function SignupScreen() {
  ...
  // Passo 2: EndereÃ§o (Completar Cadastro)
  ...
  const handleNextStep = async () => {
    ...
    if (!nome || celularLimpo.length < 10 || senha.length < 6) {
      Alert.alert('Erro', 'Preencha os dados obrigatÃ³rios corretamente.');
      return;
    }
    ...
    setLoading(true);
    try {
      // Cria o usuÃ¡rio primeiro
      await authService.signup({
        nome,
        telefone: celularLimpo,
        password: senha
      });
      // AvanÃ§a para o passo opcional
      setStep(2);
    } catch (error: any) {
      const msg = error?.response?.data?.detail || 'NÃ£o foi possÃ­vel iniciar o cadastro.';
    ...
    <RNView style={styles.outerContainer}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.container}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
          ...
          <Text style={styles.title}>{step === 1 ? 'Dados Pessoais' : 'Seu EndereÃ§o'}</Text>
          <Text style={styles.subtitle}>
            {step === 1 ? 'Primeiro, as informaÃ§Ãµes de acesso.' : 'Para finalizar a conta, informe sua localizaÃ§Ã£o.'}
          </Text>
          ...
          {step === 1 ? (
            <RNView style={styles.form}>
              ...
              <TextInput style={styles.input} placeholder="MÃ­nimo 6 caracteres" secureTextEntry value={senha} onChangeText={setSenha} />
