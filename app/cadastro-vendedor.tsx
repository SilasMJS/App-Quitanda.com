...
export default function CadastroVendedorScreen() {
    ...
    if (cepLimpo.length === 8) {
      try {
        ...
        if (!data.erro) {
          ...
          setEstado(data.uf);
          // O foco vai para o nÃºmero automaticamente (opcional)
        } else {
          Alert.alert('CEP não encontrado', 'Verifique o nÃºmero digitado.');
        }
      } catch (error) {
      ...
      try {
        ...
        if (user.tipo && user.tipo.toUpperCase() === 'VENDEDOR') {
          try {
            ...
            const vendedorInfo = resVendedores.data.find((v: any) => v.usuario_id === user.id);
            if (vendedorInfo) {
              setIsEditing(true);
              setNomeFantasia(vendedorInfo.nome_fantasia || '');
              setDescricao(vendedorInfo.descricao || '');
              setChavePix(vendedorInfo.chave_pix || '');
              if (vendedorInfo.imagem_url) setImagemUrl(vendedorInfo.imagem_url);
              
              const comInfo = listaComunidades.find((c: any) => c.id === vendedorInfo.comunidade_id);
  ...
  const handlePickImage = async () => {
    try {
      ...
      if (uri) {
        setLoading(true);
        const finalUrl = await uploadImage(uri, 'quitandas');
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      console.error(error);
      Alert.alert('Erro', error.response?.data?.detail || 'Houve um erro ao processar sua requisição.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <View style={styles.outerContainer}>
      ...
      {/* CabeÃ§alho Superior Padronizado */}
      ...
      >
        <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.header}>
            <Text style={styles.title}>{isEditing ? 'Minha Quitanda' : 'Perfil de Vendedor'}</Text>
            <Text style={styles.subtitle}>{isEditing ? 'Atualize os dados e a logo da sua quitanda' : 'Complete as informações da sua quitanda'}</Text>
          </View>

          <View style={[styles.section, { alignItems: 'center', backgroundColor: 'transparent', elevation: 0, shadowOpacity: 0 }]}>
            <TouchableOpacity onPress={handlePickImage} style={{ width: 120, height: 120, borderRadius: 60, backgroundColor: '#f0f0f0', justifyContent: 'center', alignItems: 'center', overflow: 'hidden', borderWidth: 2, borderColor: '#01A66F' }}>
              {imagemUrl ? (
                <Image source={{ uri: imagemUrl }} style={{ width: 120, height: 120 }} />
              ) : (
                <Ionicons name="camera" size={40} color="#ccc" />
              )}
            </TouchableOpacity>
            <Text style={{ fontSize: 12, color: '#666', marginTop: 8, fontWeight: 'bold' }}>
              Logo da Quitanda
            </Text>
          </View>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.row}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.inputLabel}>NÂº</Text>
          ...
          <View style={styles.section}>
            ...
            <TextInput style={styles.input} placeholder="Ex: Quitanda do ZÃ©" value={nomeFantasia} onChangeText={setNomeFantasia} />

            <Text style={styles.inputLabel}>DescriÃ§Ã£o (Opcional)</Text>
            <TextInput 
              style={[styles.input, { height: 80, textAlignVertical: 'top' }]} 
              placeholder="Fale um pouco sobre o que vocÃª vende..." 
              multiline 
              numberOfLines={3}
              value={descricao} 
            ...
            <Text style={styles.inputLabel}>Comunidade (Onde vocÃª atua?)</Text>
          ...
          >
            {loading ? (
              ...
              <Text style={styles.buttonText}>{isEditing ? 'Salvar Alterações' : 'Finalizar Cadastro'}</Text>
            )}
          </TouchableOpacity>
